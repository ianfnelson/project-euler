using System;
using System.Collections.Generic;
using System.Linq;

namespace EulerLib.Problems
{
    public class Problem0008 : IProblem
    {
        private const string SearchText =
            @"";

        public int Id
        {
            get { return 8; }
        }

        public string Title
        {
            get { return "Largest product in a series"; }
        }

        public string Solve()
        {
            return LargestProductOfDigits(SearchText, 13).ToString();
        }

        public string Md5OfSolution
        {
            get { return "0f53ea7949d32ef24f9186207600403c"; }
        }

        public long LargestProductOfDigits(string text, int digits)
        {
            var maxProduct = 0L;
            var textLength = text.Length;

            for (var pointer = 0; pointer < textLength - (digits - 1); pointer++)
            {
                var substring = text.Substring(pointer, digits);
                var productOfDigits = ProductOfDigits(substring);

                if (productOfDigits > maxProduct) maxProduct = productOfDigits;
            }

            return maxProduct;
        }

        public long ProductOfDigits(string text)
        {
            return text.Aggregate(1L, (current, c) => current * int.Parse(c.ToString()));
        }
    }
}